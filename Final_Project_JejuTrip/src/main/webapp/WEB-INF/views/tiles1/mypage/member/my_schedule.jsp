...
<link rel="stylesheet" href="<%=ctxPath%>/resources/css/mypage/member/my_schedule.css"/>
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
     ...
     //	googleCalendarApiKey : "자신의 Google API KEY 입력",
        /*
           ...
           14. "API  API 및 서비스" 에서 "라이브러리" 를 클릭한다.
               ...
               "사용" 버튼을 클릭한다. 
        */
    	googleCalendarApiKey : "AIzaSyAXU4qrLtEUmogniWAcHTg68VWHWOUL8xU",
        eventSources :[ 
            {
     ...
     // === 구글캘린더를 이용하여 대한민국 공휴일 표시하기 끝 === //
    	...
    	events:function(info, successCallback, failureCallback) {
	    	 ...
	    	 $.ajax({
                 url: '<%= ctxPath%>/selectSchedule.trip',
	                                		   ...
	                                		   if($("input:checkbox[name=my_smcatgono]:checked").eq(i).val() == item.fk_smcatgono && item.fk_userid == "${sessionScope.loginuser.userid}" ){
	   			                               //  alert("캘린더 소분류 번호 : " + $("input:checkbox[name=my_smcatgono]:checked").eq(i).val());
	                                			   events.push({
	   			                                        	    ...
	   			                                        	    url: "<%= ctxPath%>/schedule/detailSchedule.trip?scheduleno="+item.scheduleno,
	   			                                                color: item.color,
        ...
        dateClick: function(info) {
      	 // alert('클릭한 Date: ' + info.dateStr); // 클릭한 Date: 2021-11-20
      	    ...
      	    frm.method="POST";
      	    frm.action="<%= ctxPath%>/schedule/insertSchedule.trip";
      	    frm.submit();
      	  },
...
function showmyCal(){
	$.ajax({
		 url:"<%= ctxPath%>/schedule/showMyCalendar.trip",
		 type:"get",
		 ...
		 success:function(json){
			 ...
			 if(json.length > 0){
				 ...
				 $.each(json, function(index, item){
					 ...
					 html += "<td style='width:20%; padding: 3px 0px;'></td>"; 
					 html += "<td style='width:20%; padding: 3px 0px;'></td>";
				     html += "</tr>";
				 });
...
function goSearch(){
    ...
    frm.method="get";
    frm.action="<%= ctxPath%>/schedule/searchSchedule.trip";
    frm.submit();
