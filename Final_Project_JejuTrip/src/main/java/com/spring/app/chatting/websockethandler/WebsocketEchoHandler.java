...
public class WebsocketEchoHandler extends TextWebSocketHandler {
	...
	// init-method (websocketContext.xml에서 init 부분 오류나서 빈껍데기(초기화) 생성
    ...
    */
	...
	public void afterConnectionEstablished(WebSocketSession wsession) throws Exception {
       ...
       */
		...
		// System.out.println("====> 웹채팅확인용 : " + wsession.getId() + "님이 접속함");
		// ====> 웹채팅확인용 : 2dafdfea-9168-319f-3895-92f5b8877687님이 접속함
		// ====> 웹채팅확인용 : 7dd2fe55-da39-5e0f-5425-74b5b90c040b님이 접속함
		
		// System.out.println("====> 웹채팅확인용 : " + "연결 컴퓨터명 : " + wsession.getRemoteAddress().getHostName());
		// ====> 웹채팅확인용 : 연결 컴퓨터명 : DESKTOP-UOH4636
		...
		// ====> 웹채팅확인용 : 연결 IP : 192.168.0.197
		
		connectedUsers.add(wsession); // 여기에 쌓아진다 
       ...
       */
		...
		for(WebSocketSession webSocketSession : connectedUsers) {
            ...
            */
			...
			connectingUserName += loginuser.getUser_name() + " "; // 공백을 포함시켜서 쌓음
		...
		// ~~~ 확인용 connectingUserName : 「삼정수 」
		// ~~~ 확인용 connectingUserName : 「삼정수 엄정화 」
        ...
        SimpleDateFormat sdfrmt = new SimpleDateFormat("yyyy년 MM월 dd일 E요일", Locale.KOREAN); // Locale. ==> java.util import
    ...
    */
	...
	public void handleTextMessage(WebSocketSession wsession, TextMessage message) throws Exception {
	    ...
	    */
	    ...
	    MessageVO messageVO = MessageVO.convertMessage(message.getPayload()); // .getPayload()는  TesxMessage message 를 String 으로 바꿔준다
	    /* 
	    ...
	    */
	    ...
	    // %tl              시간을 1~12 으로 출력
	    // %tM              분을 00~59 으로 출력
	    // 즉, 채팅에 나오는 보내진 시간을 오전 9:59 이런식으로 표현하게 해주는 것이다
	    ...
	    for(WebSocketSession webSocketSession : connectedUsers) {
	        ...
	        if("all".equals(messageVO.getType())) {
	            ...
	            if( !wsession.getId().equals(webSocketSession.getId()) ) { 
	                 ...
	                 // 이것이 채팅메시지인데 display='none' 으로 귓속말을 위한 웹소켓아이디를 숨겨서 넣어둔 것이다                                                                                                                                                                                                                                                                                                            /* word-break: break-all; 은 공백없이 영어로만 되어질 경우 해당구역을 빠져나가므로 이것을 막기위해서 사용한다. */ 
	            }
